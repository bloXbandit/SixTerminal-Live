"""
fix_model.py  --  Run once to restore engine/schedule_model.py to a
                  CFA-trusted state after it was edited by an external process.
...
TARGET = ROOT / "engine" / "schedule_model.py"
...
class Project:
    ...
    def get_activity(self, uid=None, activity_id=None):
        if uid:
    ...
    def llm_context(self, max_activities: int = 120) -> str:
        """Thin wrapper - actual enriched context built in server.py."""
        from server import build_llm_context
        return build_llm_context(self, max_activities)
'''
